...
export function convertOddsDcHt(date, data) {
    const type = cacheGet(`${date}_ht_dc_type`) === 3 ? 3 : 0;
    const result = [];
    for (const item of data) {
        if (
            ...
            for (const match of matches) {
                ...
                const ftDCOdds = dcOdds(matchId, ticketMarketType.DoubleChance1stPeriod, oddHD, oddHA, oddAD);
                if (ftDCOdds != null) {
    ...
    if (result.length > 0) {
        if (type === 0) {
            cacheSet(`${date}_ht_dc_type`, 3, 10 * 60);
            return mergeByFixtureId(result);
        }

        const needRemoveLine = cacheGet(`${date}_ht_dc_remove_old_line`) ?? false;
        if (!needRemoveLine) {
            cacheSet(`${date}_ht_dc_remove_old_line`, true, 60);
