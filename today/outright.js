...
import {convertOddsOutright} from "../converter/outright_converter.js";
...
async function run() {
    ...
    mq.configure({queue: 'outright_fbc'});
    await mq.connect();
        ...
        await frame.evaluate((prefix) => {
            ...
            const extractData = () => {
                ...
                for (const row of rows) {
                    // League header
                    if (row.querySelector(".leagueName")) {
                    ...
                    const time = cleanText(
                        row.querySelector("td.time")?.textContent
                    )
    ...
    async function initObserver() {
        ...
        const targetFrame = page
            .frames()
            .find(frame => frame.url().includes("Handicap/Outright.aspx"));

        if (!targetFrame) return false;
        await injectObserver(targetFrame);
        console.log(`${targetFrame.url()}- ${new Date().toISOString()} -Outright observe!`);
        return true;
    }

    async function goToOT() {
        ...
        await targetFrame.evaluate(() => {
            document.querySelector('[id="1_OT"] a').click();
        });
    }
