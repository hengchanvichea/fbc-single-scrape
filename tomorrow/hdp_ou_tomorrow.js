...
async function run() {
    ...
    await page.exposeFunction("pushOddsToNode", async (payload) => {
        try {
            if (payload != null) {
                ...
                const results = convertOddsHdpOu("tomorrow", data);
    ...
    async function initObserver() {
        ...
        const targetFrame = page
            .frames()
            .find(frame => frame.url().includes("Handicap/Early.aspx"));

        if (!targetFrame) return false;
        await injectObserver(targetFrame);
        console.log(`${targetFrame.url()}- ${new Date().toISOString()} -HDP OU Tomorrow observe!`);
        return true;
    }

    async function goToEarly() {
